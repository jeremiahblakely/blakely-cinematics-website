...
async function getFolderCounts(userId) {
    const folders = ['inbox', 'starred', 'sent', 'drafts', 'bookings', 'clients', 'finance', 'galleries', 'trash', 'archive', 'snoozed'];
    const counts = {};
    ...
    for (const folder of folders) {
        try {
            ...
            console.error(`Error counting ${folder}:`, error);
            counts[folder] = 0;
        }
    }
...
exports.handler = async (event) => {
    ...
    // Handle folder counts endpoint
    if (event.path === '/mail/folders/counts' && event.httpMethod === 'GET') {
